...
// js -> fileDown()
	String b_file = request.getParameter("b_file"); // 쿼리스트링으로 넘어온 파일명 담기
	String fname = b_file;
	//out.print("b_file: 8->euc"+b_file);		
	//out.print("<br>");		
	// 아래 경로는 톰캣서버가 바라보는 프로젝트 내의 pds폴더를 가리킴
	String filePath = "D:\\workspace_jsp\\basic0512\\src\\main\\webapp\\pds"; // 절대경로.	
	//가져온 파일이름을 객체화 시켜줌. - 파일이 있는 물리적인 경로가 필요함.
	File file = new File(filePath,b_file.trim()); // trim(): 공백을 제거하는 메서드
	...
	try{
		...
		//사용한 자원은 반납해줌. - 묵시적으로 처리됨 - 명시적으로 처리하는것 권장사항 - 시점을 앞당김 - 서버에 대한 부담을 줄임
		// 보안상으로 닫지 않으면 악의적인 목적으로 가로채거나 인터셉트한 후에 위변조를 취함
