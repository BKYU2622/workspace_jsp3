<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	// 스크립틀릿 - 자바코드를 작성할 수 있는 영역, 여기서는 메서드 선언 불가, 여기서 선언한 변수는
	// 서블릿 코드의 doService 메서드 안에 들어가므로 모두 지변이다. 
	// 인스턴스화나 변수 선언, 제어문 사용은 가능함
...
<nav class="navbar navbar-expand-lg bg-light">
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav">
        <li class="nav-item">
          <a class="nav-link active" aria-current="page" href="/auth2/loginForm.jsp">로그인</a><!-- active가 좀 더 진하게 보이게 해줌 -->
        </li>
         <li class="nav-item">
      ...
      <!-- 로그인 성공시 세션에 담긴 이름을 출력하고 로그인 성공한 경우이므로 로그아웃 버튼을 추가함 -->
<%
	if(nickname != null) {            
%>        
	...
	<!-- 
	button을 클릭했을 때 onclick은 이벤트 핸들러의 이름이다
	클릭이 되면 느끼는 것은 브라우저 화면에서 감지하는 건 
	js처리를 해야 함 - 이벤트 처리 담당은 자바 스크립트 쪽에서 처리함
	 --> 
