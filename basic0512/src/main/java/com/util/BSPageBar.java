...
public class BSPageBar {
	...
	//전체레코드 갯수 -> 리스트의 수를 세는 것
	private int totalRecord;//list.size():47row
	
	//페이지당 레코드 수(한 페이지에 10개씩)
	...
	//현재 내가 바라보는 페이지 수 - ex) 15번이면 2page
	private int nowPage;
	
	//현재 내가 바라보는 블럭 수 - 0부터 시작
	 ...
	 * 생성자 선언(파라미터가 4개가 필요해, 왜냐하면 생성자는 메서드 오버로딩의 규칙을 따르니까)
	 */
	 ...
	 * @param totalRecord - List.size(); -> row의 수이다
	 * @param nowPage - 현재 내가 바라보는 페이지
	 * @param pagePath - 어떤 페이지에서 이 공통 코드를 사용할 것인가
	 *******************************************************************************************/
	// 이 페이지네이션을 사용하는 페이지마다 몇 개씩 보여줄지, 페이지를, 전체 레코드 수, URL 당연히 달라야 하지 않나?
	// 해당 페이지에서 생성자의 파라미터로 받아온 정보는 최소한 같은 주소번지에 대해서는 모두 일치해야 한다 - 이랬다 저랬다 하면 안 됨
	
	public BSPageBar(int numPerPage, int totalRecord, int nowPage, String pagePath) {
		this.numPerPage = numPerPage; // 한 페이지에 몇 개의 로우를 뿌릴 것인가
	...
	//setter메소드 선언
	// 페이징 처리로 필요한 화면(Tag)을 그려주는 메서드이다.
	public void setPageBar() {
		// 문자열을 조작할 때 원본이 바뀌지 않는 String클래스 대신에 StringBuilder(싱글스레드에 안전 - 속도 비교적 빠름)나 StringBuffer(멀티스레드에 안전 - 속도는 비교적 느림)클래스를 사용하자
		StringBuilder pageLink = new StringBuilder(); // 싱글스레드여도 괜찮음
		//전체 레코드 수가 0보다 클때 처리하기 - 조회된 글이나 등록된 글이 있을 때만 페이지 네비게이션이 필요함
		if(totalRecord>0) {
			//nowBlock이 0보다 클때 처리 - 화살표를 의미함 - ex) 1 2 3 > 이 0블럭, < 4 5 6 >이 1블럭 
			...
			if(nowBlock > 0 ) {                                    //(1-1)*2+(2-1)=1
				...
				pageLink.append("</a>"); // a태그는 마우스를 대면 손 모양이 뜸 => 링크
				pageLink.append("</li>");
			}
			// 전체 레코드 수가 65개이고 그 중에서 내가 45번 글을 보고 있을 때 라고 가정한다
			for(int i=0;i<pagePerBlock;i++) { // 전체 페이지 블록 수는 3개 이다 - 1 2 3>> <<4 5 6>> <<7
				//현재 내가 보고 있는 페이지 블록 일때와
				if(nowBlock*pagePerBlock+i==nowPage) {
					// 현재 내가 보고 있는 2번째 페이지 블록에서 5번 페이지를 보고 있다면 5번 숫자에 대해서는 링크가 필요 없다
				...
				if((nowBlock*pagePerBlock)+i+1==totalPage) break; // 2*3+0+1=7, 2*3+1+1=8, 2*3+2+1=9
			}// end of for문 - 3바퀴 반복
		...
		pagination = pageLink.toString(); // toString()을 붙인 이유는 pageLink가 StringBuilder타입이기 때문
	}
	//getter메소드 선언
	// 
	public String getPageBar() { // getPageBar()의 리턴타입이 String이니까 print메서드 안에서 호출이 가능
		this.setPageBar();
		// 그 때마다 달라져야 한다 - 인스턴스화 할 때마다 달라져야 함 -> BSPageBar pb = new BSPageBar();
